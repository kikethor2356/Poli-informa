...
function handle_login_attempt($conexion, $codigo) {
    $stmt = $conexion->prepare("SELECT login_attempts, last_attempt FROM registroalu WHERE CodeAlu=?");
...
function increment_login_attempts($conexion, $codigo) {
    $stmt = $conexion->prepare("UPDATE registroalu SET login_attempts = login_attempts + 1, last_attempt = NOW() WHERE CodeAlu = ?");
    $stmt->bind_param("s", $codigo);
    $stmt->execute();
}

function reset_login_attempts($conexion, $codigo) {
    $stmt = $conexion->prepare("UPDATE registroalu SET login_attempts = 0, last_attempt = NULL WHERE CodeAlu = ?");
    $stmt->bind_param("s", $codigo);
    $stmt->execute();
}
?>