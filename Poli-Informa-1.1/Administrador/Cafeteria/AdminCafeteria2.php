...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css" integrity="sha512-DTOQO9RWCH3ppGqcWaEA1BIZOC6xxalwEsw9c2QQeAIftl+Vegovlnee1c9QX4TctnWMn13TZye+giMm8e2LwA==" crossorigin="anonymous" referrerpolicy="no-referrer" />    <title>Administracion Cafeteria</title>
...
<?php
    ...
    <div class="ventanaEmergente">
        <div class="contenidoVentanaEmergente">
            ...
            <form action="Agregar/InsertarDatosCafeteria2.php" method="POST" enctype="multipart/form-data">
    ...
    <div class="ventanaEditarProducto">
        <div class="contenidoVentanaEditarProducto">
            ...
            <form action="Agregar/EditarDatosCafeteria2.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" id="cafeteriacanchas_id" name="cafeteriacanchas_id">
                ...
                <div id="contenedor-imagen1">
                    ...
                    <img src="" alt="" id="imagenPreviewEditar"><br><br>
        ...
        <main id="principal-productos">
            <section id="section-productos">
                <div id="mostrarProductos">
                    <center><h2>CAFETERIA CANCHAS</h2></center>
                    ...
                    <div id="paginacion">
                        ...
                        $sql = "SELECT COUNT(*) AS total FROM cafeteriacanchas";
                        $resultado = mysqli_query($conexion, $sql);
                        $fila = mysqli_fetch_assoc($resultado);
                        $total_resultados = $fila['total'];
                        $total_paginas = ceil($total_resultados / $resultados_por_pagina);

                        $sql = "SELECT * FROM cafeteriacanchas INNER JOIN categorias_cafeteria ON cafeteriacanchas.prodcategoria_id = categorias_cafeteria.categoria_id LIMIT $inicio, $resultados_por_pagina";
                        $resultado = mysqli_query($conexion, $sql);
                        ?>
                    </div>
                    
                    <!-- Datos para registrar de cafeteria canchas -->
                                ...
                                <tr>
                                    ...
                                    <td id="cabezaAcciones" class="<?php echo $clase_fila; ?>">
                                        <div class="acciones-buttons">
                                            ...
                                            <button type="button" class="iconoEditar" title="Editar registro" onclick="abrirVentanaEditar('<?php echo $row['cafeteriacanchas_id']; ?>', '<?php echo $row['nombre_producto']; ?>', '<?php echo $row['descripcion']; ?>', '<?php echo $row['precio']; ?>', '<?php echo $row['imagen']; ?>', '<?php echo $row['categoria_nombre']; ?>', '<?php echo $row['prodcategoria_id']; ?>', '<?php echo $row['imagen']; ?>')"><i class="fa-solid fa-pen-to-square"></i></button>
                                            <form id="eliminarForm_<?php echo $row['cafeteriacanchas_id']; ?>" action="Agregar/EliminarCafeteria2.php" method="POST">
                                                <input type="hidden" name="eliminar_id" value="<?php echo $row['cafeteriacanchas_id']; ?>">
                                                ...
                                                <button type="button" class="iconoEliminar" title="Eliminar registro" onclick="mostrarConfirmacion('<?php echo $row['cafeteriacanchas_id']; ?>')"><i class="fa-solid fa-trash-can"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
