...
function handle_login_attempt($conexion, $codigo) {
    $stmt = $conexion->prepare("SELECT login_attempts, last_attempt, unlock_time FROM registro WHERE AdCode=?");
...
function increment_login_attempts($conexion, $codigo) {
    $stmt = $conexion->prepare("SELECT login_attempts FROM registro WHERE AdCode = ?");
    $stmt->bind_param("s", $codigo);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result && $result->num_rows > 0) {
        ...
        $stmt = $conexion->prepare("UPDATE registro SET login_attempts = ?, last_attempt = NOW(), unlock_time = ? WHERE AdCode = ?");
...
function reset_login_attempts($conexion, $codigo) {
    $stmt = $conexion->prepare("UPDATE registro SET login_attempts = 0, last_attempt = NULL, unlock_time = NULL WHERE AdCode = ?");
    $stmt->bind_param("s", $codigo);
    $stmt->execute();
}
?>